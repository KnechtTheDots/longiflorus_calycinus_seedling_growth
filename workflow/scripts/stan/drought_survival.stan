...
transformed data{
  vector[n_long] log_area = log(area); // log of area (exponential growth)
  vector[n_short] seed_std = (seed - mean(seed))/sd(seed); // standardized seed size for initial size submodel
  vector[n_short] size_std = (final_size - mean(final_size))/sd(final_size); // standardize final size for the logistic regression
}
parameters{
  ...
  real rgr_bar; // mean relative growth rate
...
model{
  ...
  survive ~ bernoulli_logit(alpha_survive + beta_survive * size_std);
}
generated quantities{
  ...
  array[n_short] real y_pred = normal_rng(log_size_0[id_short] + rgr[id_short] .* age_max, sig_m);
